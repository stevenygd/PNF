...
def get_opt(params, cfgopt, overwrite_lr=None):
    ...
    if cfgopt.type == 'adam':
        optimizer = optim.Adam(
            ...
            amsgrad=getattr(cfgopt, "amsgrad", False)
        )
    ...
    if scheduler_type is not None:
        if scheduler_type == 'exponential':
            ...
            def lambda_rule(ep):
                lr_l = 1.0 - min(
                    1,
                    max(0, ep - start_ratio * step_size) / float(duration_ratio * step_size)
                ) * (1 - final_ratio)
                return lr_l

            scheduler = optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule)

        elif scheduler_type == 'bacon_log':
            steps = int(getattr(cfgopt, "step_epoch", 2000))
            final_ratio = float(getattr(cfgopt, "final_ratio", 0.01))
            start_ratio = float(getattr(cfgopt, "start_ratio", 0.01))
            lr0 = float(start_ratio)
            lrn = float(final_ratio)

            # https://github.com/computational-imaging/bacon/blob/main/training.py
            def lr_log_schedule(it, num_steps=1e6, nw=2500, lr0=1e-3, lrn=5e-6, lambdaw=0.01):
                it += 1
                lr_it = (lambdaw + (1 - lambdaw) * np.sin(np.pi / 2 * np.clip(it / nw, 0, 1))) * \
                        np.exp((1 - it / num_steps) * np.log(lr0) + (it / num_steps) * np.log(lrn))
                # ratio = lr_it / float(lr)
                ratio = lr_it
                # print(it, lr_it, ratio)
                return ratio

            log_scheduler = partial(
                lr_log_schedule, num_steps=steps, nw=1, lr0=lr0, lrn=lrn)

            scheduler = optim.lr_scheduler.LambdaLR(
                optimizer, lr_lambda=log_scheduler)

        # elif scheduler_type == 'bacon_log_nerf':
        #     steps = int(getattr(cfgopt, "step_epoch", 2000))
        #
        #     # https://github.com/computational-imaging/bacon/blob/main/training.py
        #     def lr_log_schedule(it, num_steps=1e6, nw=2500, lr0=1e-3, lrn=5e-6, lambdaw=0.01):
        #         it += 1
        #         lr_it = (lambdaw + (1 - lambdaw) * np.sin(np.pi / 2 * np.texture(it / nw, 0, 1))) * \
        #                 np.exp((1 - it / num_steps) * np.log(lr0) + (it / num_steps) * np.log(lrn))
        #         # ratio = lr_it / float(lr)
        #         ratio = lr_it
        #         # print(it, lr_it, ratio)
        ...
        #
        #     log_scheduler = partial(
        #         lr_log_schedule, num_steps=steps)
        #
        #     scheduler = optim.lr_scheduler.LambdaLR(
        #         optimizer, lr_lambda=log_scheduler)
...
from utils import load_imf, dict2namespace